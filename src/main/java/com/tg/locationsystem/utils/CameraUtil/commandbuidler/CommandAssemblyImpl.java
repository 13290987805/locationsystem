package com.tg.locationsystem.utils.CameraUtil.commandbuidler;
		...
		try {
			if (paramMap.containsKey("ffmpegPath")) {
				...
				if (paramMap.containsKey("input") && paramMap.containsKey("output") && paramMap.containsKey("appName")
					...
					// 当twoPart为0时，只推一个元码流
					if ("0".equals(twoPart)) {
						//comm.append(" -vcodec " + codec + "-fflags nobuffer -f flv -an " + output + appName);
						comm.append("-vcodec"+codec+ " -b:v 400k -s "+rs1+" -r 25 -acodec libfaac -b:a 64k -f flv -an "+ output + appName );
					} else {
						// -f ：转换格式，默认flv
